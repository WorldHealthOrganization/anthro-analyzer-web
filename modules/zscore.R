...
zscore <- function(
  ...
  validation <- callModule(
    validatorModule,
    "validation",
    reactive_zscore_validation(dataset, list_map_vars)
  )
  ...
  output$z <- DT::renderDataTable(
    {
      ...
      data <- dplyr::select(local_zscores(), -uid)
  ...
  df_download_z <-
    reactive({
      req(is.data.frame(local_zscores()))
      bind_rows(
        mutate(local_zscores(), included = TRUE),
        df_age_group() %>%
          anti_join(local_zscores(), by = "uid") %>%
  ...
  output$dl_zscore <- downloadHandler(
    filename = function() {
      make_export_filename(original_filename(), "zscore.csv")
    },
