PlotDigitOutput <- function(id) {
  ...
  tabPanel(
    ...
    tags$style(
      type = "text/css",
      "#dl_digit { background-color:LightGrey; float:right; margin-right: 15px;}"
    ),
...
PlotDigit <- function(
  ...
  validation <- callModule(
    ...
    reactive({
      check_validation_conditions(
        ...
        condition(
          !(list_map_vars[["weight"]]() != "None" &&
            list_map_vars[["lenhei"]]() != "None"),
          validation_error(
  ...
  digit_preference <- reactive({
    ...
    digit_prepare_heaping_plot_data(
      df_zscores(),
  ...
  output$dynamic_plot_digit <- renderUI({
    plotOutput(
      ns("plot_digit"),
      height = paste0(digit_heaping_plot_height(), "px")
    )
  })
}
