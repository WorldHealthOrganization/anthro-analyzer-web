PlotDistOutput <- function(id) {
  ...
  tabPanel(
    ...
    fluidRow(
      column(6, VariablesInput(ns("id_variables_x"))),
      column(6, VariablesInput(ns("id_variables_strat_dist")))
    ),
    fluidRow(
      column(6, plotOutput(ns("plot_dist"))),
      column(6, uiOutput(ns("mismatch_table")))
    )
  )
}
...
PlotDist <- function(
  ...
  mismatch_table_data <- reactive({
    ...
    data <- dist_compute_mismatch_table(
      plot_data(),
      measure_col,
  ...
  output$dl_mismatch_table <- downloadHandler(
    filename = function() {
      make_export_filename(original_filename(), "mismatch_table.csv")
    },
  ...
  output$mismatch_table <- renderUI({
    if (show_mismatch_table()) {
      list(
        ...
        br(),
        renderTable(mismatch_table_data()),
        p(paste0(
