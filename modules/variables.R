...
Variables <- function(
  ...
  stopifnot(
    is.null(choices) || shiny::is.reactive(choices) || is.character(choices)
  )
  ...
  output$ui_variables <- renderUI({
    element <- selectInput(
      ...
      label = HTML(label), # label is trusted input
  ...
  reactive({
    value <- input$slct_variables
    req(all(value %in% "None" | value %in% choice_input()))
    value
  })
}
