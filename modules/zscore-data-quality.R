ZscoreDataQualityModuleOutput <- function(id) {
  ...
  list(
    tabPanel(
      ...
      fixedRow(
        column(12, VariablesInput(ns("id_variables_strat_z_zscore")))
      ),
...
ZscoreDataQualityModule <- function(
  input,
  output,
  session,
  list_map_vars,
  ...
  callModule(
    validatorModule,
    "validation_dist",
    reactive_zscore_validation(dataset, list_map_vars, height_and_weight = TRUE)
  )
  callModule(
    validatorModule,
    "validation_flags",
  ...
  zscores_dist_data <- reactive({
    ...
    req(is.data.frame(local_zscores()))
    zscore_prepare_plot_dist_data(
      df_zscores = local_zscores(),
  ...
  validate_zscore_plot <- function(n_levels) {
    validate(
      need(
        is.na(n_levels) || n_levels < 30,
        "This plot is only available for less than 30 groups"
      )
    )
  }
  ...
  zscores_wo_flagged <- reactive({
    req(is.data.frame(local_zscores()))
    set_flagged_zscores_to_na(local_zscores())
  })
  ...
  observeEvent(local_zscores(), {
    if (is.data.frame(local_zscores())) {
      fun <- shinyjs::enable
    } else {
      fun <- shinyjs::disable
    }
  ...
  output$dl_zscore_summary <- downloadHandler(
    filename = function() {
      make_export_filename(original_filename(), "zscore_summary.csv")
    },
  ...
  output$zscore_summary <- DT::renderDataTable({
    ...
    for (indicator in zscore_summary_cols) {
      res <- DT::formatStyle(
        res,
        zscore_create_col_labels(indicator),
