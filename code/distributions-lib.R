...
dist_prepare_plot_data <- function(dataset) {
  ...
  dataset$plot_age_in_years <- cut(
    dataset$age_in_months,
    breaks = c(0, 12, 24, 36, 48, 60),
    right = FALSE,
    labels = 0:4
  )
  dataset$plot_age_in_months <- cut(
    dataset$age_in_months,
    0:60,
    right = FALSE,
    labels = 0:59
...
dist_plot <- function(
  ...
  if (
    missing(stratification_variable) || length(stratification_variable) == 0L
  ) {
    plot_tmp <-
      if (is_x_discrete) {
        # bar plot
    ...
    req(
      length(stratification_variable) == 1L &&
        stratification_variable %in% colnames(plot_data)
    )

    is_strat_sex <- stratification_variable_label == "Sex"
    is_strat_wiq <- is_label_wiq(stratification_variable_label)

    plot_data[[stratification_variable]] <- factor(
      as.character(plot_data[[stratification_variable]]),
      levels = sort(unique(plot_data[[stratification_variable]]))
    )
    plot_data[[stratification_variable]] <- forcats::as_factor(plot_data[[
      stratification_variable
    ]])
    if (is_strat_sex) {
      plot_data[[stratification_variable]] <- recode_sex_factor(plot_data[[
        stratification_variable
      ]])
    } else if (is_strat_wiq) {
      plot_data[[stratification_variable]] <- recode_wiq_factor(plot_data[[
        stratification_variable
      ]])
    }
    plot_tmp <-
      if (is_x_discrete) {
        ggplot(plot_data) +
          aes(
            ...
            fill = !!as.symbol(stratification_variable)
          ) +
          geom_bar() +
          ...
          scale_fill_manual(
            ...
            values = gray.colors(
              dplyr::n_distinct(plot_data[[stratification_variable]]),
              end = 0.7
            )
          ) +
          labs(x = x_variable_label)
...
dist_compute_mismatch_table <- function(data, measure_col, age_in_month_col) {
  age_groups <- list(
    ...
    as.integer(c(
      0,
      max(data[[as.character(age_in_month_col)]], na.rm = TRUE) + 100
    ))
  )
  ...
  res <- res[, c(1, 5, 2, 3, 4)]
  res <- dplyr::mutate(
    dplyr::filter(res, is.finite(rel)),
    rel = scales::percent(rel, accuracy = 0.1)
  )
  colnames(res) <- c(
    "Age group",
    "Expected position",
