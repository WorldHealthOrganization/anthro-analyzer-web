...
#' Returns a data frame with a row per flag.
#' Each row has the proportion of zscores flagged and the numerator.
zscore_flagged_zscores <- function(df_zscores) {
  df_zscores <- set_flagged_zscores_to_na(df_zscores)
  numerators <- df_zscores %>%
  ...
  proportions <- df_zscores %>%
    select(one_of(zscore_flag_cols)) %>%
    summarise_all(
      list(
        ~ sum(. == 1, na.rm = TRUE) / sum(!is.na(.)) * 100
      )
    ) %>%
    gather(value = "value")

  inner_join(proportions, numerators, by = "key")
}

zscore_plot_flagged_zscores <- function(df_zscores) {
  plot_data <- zscore_flagged_zscores(df_zscores)
  plot_data %>%
    ggplot(aes(x = fct_rev(key), y = value)) +
    geom_segment(aes(xend = fct_rev(key), yend = 0)) +
    geom_point(size = 1) +
    geom_text(
...
zscore_prepare_plot_dist_data <- function(
  df_zscores,
  ...
  flag_available <-
    df_zscores %>%
    summarise_at(vars(matches("^z.+_flag$")), funs(all(is.na(.)))) %>%
  ...
  strat_row <- df_strat[
    df_strat$value == stratification_var_name,
    ,
    drop = FALSE
  ]
...
zscore_plot_distribution <- function(data) {
  ...
  if (
    !is.null(stratification_var_name) &&
      is_label_wiq(stratification_var_name) &&
      is.factor(df_zscore_dist[[stratification_var_name]])
  ) {
    df_zscore_dist[[
      stratification_var_name
    ]] <- recode_wiq_factor(df_zscore_dist[[stratification_var_name]])
  }
...
set_flagged_zscores_to_na <- function(zscore_dataframe) {
  ...
  stopifnot(
    c(zscore_summary_cols, zscore_flagged_cols) %in% colnames(zscore_dataframe)
  )
...
zscore_compute_summary_data <- function(
  zscores_wo_flagged,
  zscore_summary_groups
) {
  res <- summarise_zscore_data(
    zscores_wo_flagged,
    groups = zscore_summary_groups
  )
  zscore_col_names <- unlist(lapply(
    zscore_summary_cols,
    zscore_create_col_labels
  ))
...
summarise_zscore_data <- function(data, groups = list()) {
  ...
  compute_statistics <- function(group, zscore_data) {
    result <- tibble::tibble(group, n = nrow(zscore_data))
