digit_prepare_heaping_plot_data <- function(
  df_zscores,
  weight_col,
  lenhei_col,
  strat_column
  ...
  if (!is.null(strat_column)) {
    digit_preference %<>%
      bind_cols(df_zscores[, strat_column, drop = FALSE]) %>%
...
digit_create_heaping_plot <- function(
  plot_data,
  ...
  if (!is.null(max_len) && length(max_len) == 1 && max_len > 0) {
    p <- p + ylim(0, max_len)
  }
...
digit_weight_plot <- function(plot_data) {
  digit_plot_integer_histogram(
    plot_data,
    "Weight (kg) -",
    limits = digit_weight_bounds
  )
}
...
digit_height_plot <- function(plot_data) {
  digit_plot_integer_histogram(
    plot_data,
    "Length or height (cm) -",
    limits = digit_height_bounds,
    step_size = 5
  )
}

digit_plot_integer_histogram <- function(
  plot_data,
  xlab,
  limits,
  step_size = 1
