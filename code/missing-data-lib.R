...
missing_data_prepare_plot_data <- function(
  dataset,
  df_zscores,
  ...
  df_missing <-
    count_missing_values(
      df_zscores,
      setdiff(quality_vars$value, strat),
      strat
    ) %>%
...
missing_data_prepare_missings_table <- function(
  plot_data,
...
count_missing_values <- function(data, columns, stratifications) {
  ...
  data <- dplyr::summarise_all(
    data,
    list(na = ~ sum(is.na(.)), n = ~ dplyr::n())
  )
